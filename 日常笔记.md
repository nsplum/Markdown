...
&#160; &#160; &#160; &#160; 最近打算折腾`vn.py`,但只有py27版本的，因为一向习惯使用最新稳定版的，所以不得不装py27的环境，不得不说Python 的全局锁真的很烦。身为懒癌患者，必然使用全功能的anaconda，但不想同时装py27和py35两个版本的anaconda巨无霸（同时装两个，不知道conda是否也可以管理环境），于是选择用conda装python27的环境及一些必要的包。 弄了几天终于把办公电脑和家里的Mac机上的环境都配好了，即使有了官方的安装教材，也踩了不少坑。 ( 因为国内主要的期货交易API接口只有windows和linux版，所以Mac上的环境只能用来回测，无法使用vn.py的模拟交易和实盘功能。)
...
`http://blog.csdn.net/hero_myself/article/details/55105647`
...
#### 在Xcode下使用OpenCV
1. 使用Xcode创建一个Command Line Tools，这里和创建普通的Xcode C++程序是一样的。
2. 点击工程（左上，蓝色的那个），选择TARGETS->右侧选择All->Build Settings -> 搜索框输入search paths ->选择search paths下面的Header Search Parhs ->双击后面的详细信息添加指定路径：/usr/local/include (这里有的博文说需要安装第二个路径：/usr/local/include/opencv，我在我的系统上只安装第一个就可以运行了，依情况而定吧）。之后选择search paths下面的Library Search Parhs ->双击添加指定路径:/usr/local/lib (这里包含了所需要的库文件)。
3. 在XCode的工程下建立一个NewGroup
方法：右键工程（左上，蓝色的那个），然后点击“New Group”，改变名字，例如OpenCV FrameWork。选中该文件夹，右键选Add files，然后，键盘输入`/`号，系统会弹出来一个对话框，再输入“/usr/local”之后点回车，它会转入local文件夹下，然后按如下顺序进入： Cellar-->opencv-->2.4.3-->lib， 此时可以看到一大堆以“dylib”结尾的文件（这些文件会有一些带箭头的版本，不用理会），这些就是“动态库”，然后你找到以下三个动态库（如果做比较复杂的图像工作，可以把18个库全部包含进去，这里的测试用例使用下面的3个库就可以了）：
...
### MacOS+python3+OpenCV3环境配置

安装： opencv3

用终端可以直接安装opencv，但还不能用，需要将cv2链接到python3类库目录下之后才能用。具体代码如下：
...
```
注：python2和python3一次只能装一个，用without-python可以跳过python2让opencv装在python3里。
...
注：numpy是任意import的一个在site-packages里的东西，也可以用其他的代替。

#### step3:将cv2链接到python3类库目录下
...
注：这个需要用command+d跳出python的shell，在新建终端进行。
#### step4:测试一下
...
除去经典的阈值和分水岭分割算法外，有**主动轮廓**及衍生的**水平集**，图割及相关算法（例如GrabCut），交互式分割，以及Cosegmentation。所有分割算法的核心目的是解决目标区域语义合并难题。例如交互式分割，通过精准交互来解决该问题。Cosegmentation通过分割相同或相似目标来处理该问题。

李纯明前辈(个人网址：http://www.engr.uconn.edu/~cmli/)的水平集方法。

作者：知乎用户
链接：https://www.zhihu.com/question/26085954/answer/93900416

### 基于深度学习的图像去噪
(2016-05-08 19:53:43)

http://blog.sina.com.cn/s/blog_6dd1bab30102wh1e.html

1. 2015 深度学习、自编码器、低照度图像增强
...
3. 2014 深度学习、rectified linear函数、深度神经网络、图像去噪
...
4. 2013 深度学习、堆叠式稀疏去噪自编码器SSDAs、深度神经网络DNN、图像去噪
